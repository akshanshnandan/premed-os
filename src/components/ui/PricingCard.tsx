...
export function PricingCard({ name, price, detail, features, featured, cta = "Start free" }: PricingCardProps) {
  return (
    <div
      className={cn(
        "flex h-full flex-col rounded-lg border bg-white p-6 shadow-sm",
        featured && "border-brand-300 shadow-soft ring-1 ring-brand-100",
      )}
    >
      {featured ? (
